...
description: video-spec.json의 세그먼트별 narration을 로컬 macOS TTS, OpenAI TTS 또는 ElevenLabs로 합성하고 세그먼트별 length(narration_ms) + 통합 mp3를 생성합니다. /vg-video-demo Phase 2 전용.
---

# /vg-video-demo-voice — 세그먼트별 TTS 생성
...
- 로컬 기본: macOS `say` 사용 가능 환경
- 권장: `.env`에 `OPENAI_API_KEY`
- 레거시 호환: `.env`에 `ELEVENLABS_API_KEY`, `ELEVENLABS_VOICE_ID=j9zDdWCMVw4VqUJwzwAL`
- 선택: `video-spec.json.voice`에 `provider`, `model`, `voice`, `instructions`, `speed` 지정
...
2. **세그먼트별 TTS 호출**
   - 로컬: macOS `say` + 한국어 음성(`Yuna` 등)
   - 기본: OpenAI `gpt-4o-mini-tts` + `voice`(기본 `coral`) + `instructions`
   - 호환: `provider="elevenlabs"`면 기존 PVC 설정 사용
...
- OpenAI 사용 시 과한 감정 프롬프트 금지 — 튜토리얼 톤이 깨질 수 있음
