...
7. **local(.Codex/skills/) 와 global(~/.Codex/skills/) vg-* 스킬 동기화**
   — 패키징 전 `cp -R .Codex/skills/vg-*` 로 global 을 local 과 일치시킬 것
...
내용은 현재 코드베이스, docs/, AGENTS.md를 읽고 **최신 상태**를 반영하여 작성합니다.
...
패키징 전 반드시 local(.Codex/skills/) 의 vg-* 스킬을 global(~/.Codex/skills/) 로 복사:

```bash
for s in .Codex/skills/vg-*; do
  name=$(basename "$s")
  rm -rf ~/.Codex/skills/"$name"
  cp -R "$s" ~/.Codex/skills/"$name"
done
```
...
- [ ] ZIP 의 vg-* 스킬 수가 로컬과 일치 (`[ "$(unzip -l dist/newVideoGen-v*.zip | grep -c 'skills/vg-[^/]*/skill.md')" = "$(ls -d .Codex/skills/vg-*/ | wc -l | tr -d ' ')" ]`)

## ZIP 포함 목록

| 폴더/파일 | 포함 | 이유 |
|----------|------|------|
| `.Codex/skills/` | ✅ | 핵심 스킬 파일 |
| `src/` | ✅ | 소스 코드 |
| `scripts/` | ✅ | 후처리 + 생성 스크립트 |
| `docs/` | ✅ | 문서 |
